#ifndef _nwpw_adaptive_scf_mixing_HPP_
#define _nwpw_adaptive_scf_mixing_HPP_

#pragma once

/* nwpw_adaptive_scf_mixing.hpp
   Author - AI Assistant
   
   This class extends the standard SCF mixing with adaptive parameter control
...
public:
    ...
    /*************************************************
     *                                               *
     *    nwpw_adaptive_scf_mixing::nwpw_adaptive_scf_mixing
     *                                               *
     *************************************************/
    nwpw_adaptive_scf_mixing(PGrid *mygrid0, const double g0, const int algorithm0, 
                            ...
                            const int ispin0, const int nsize0, double *rho_in) 
    {
        ...
        base_mixer = new nwpw_scf_mixing(mygrid0, g0, current_algorithm, 
                                        current_alpha, current_beta, max_m0, 
                                        ispin0, nsize0, rho_in);
        
        if (verbose_output && parall->is_master()) {
            std::cout << "        - Adaptive SCF mixing initialized with automatic parameter adjustment" << std::endl;
    ...
    ~nwpw_adaptive_scf_mixing() {
        ...
        if (verbose_output && parall->is_master()) {
            if (nan_detection_count > 0) {
                std::cout << "        - Adaptive mixing completed with " << nan_detection_count 
    ...
    /*******************************************
     *                                         *
     *    nwpw_adaptive_scf_mixing::adaptive_mix
     *                                         *
     *******************************************/
    /**
     * @brief Perform adaptive SCF mixing with automatic parameter adjustment.
     *
    ...
    bool adaptive_mix(double *vout, double *vnew, const double deltae, double *scf_error0) 
    {
        ...
        try {
            ...
            if (has_nan) {
                if (verbose_output && parall->is_master()) {
                    std::cout << "        - WARNING: NaN/Inf detected in mixing output" << std::endl;
                }
                return attempt_recovery(vout, vnew, deltae, scf_error0);
            }
            ...
            if (verbose_output && parall->is_master()) {
                std::cout << "        - EXCEPTION in mixing: " << e.what() << std::endl;
            }
            return attempt_recovery(vout, vnew, deltae, scf_error0);
        }
    }
    ...
    /*******************************************
     *                                         *
     * nwpw_adaptive_scf_mixing::attempt_recovery
     *                                         *
     *******************************************/
    /**
    ...
    bool attempt_recovery(double *vout, double *vnew, const double deltae, double *scf_error0)
    {
        ...
        if (nan_detection_count > max_nan_recovery_attempts) {
            if (verbose_output && parall->is_master()) {
                ...
                std::cout << "        - SCF calculation may be unstable" << std::endl;
            }
            return false;
        }
        ...
        if (current_alpha < 0.01 && current_algorithm != 0) {
            ...
            if (verbose_output && parall->is_master()) {
                std::cout << "        - Switching to simple mixing algorithm" << std::endl;
            }
        }
        ...
        delete base_mixer;
        base_mixer = new nwpw_scf_mixing(mygrid, 0.0, current_algorithm, 
    ...
    /*******************************************
     *                                         *
     * nwpw_adaptive_scf_mixing::reset_parameters
     *                                         *
     *******************************************/
    /**
    ...
    void reset_parameters() {
        ...
        if (verbose_output && parall->is_master()) {
            std::cout << "        - Adaptive mixing parameters reset to original values" << std::endl;
        }
    }

    /*******************************************
     *                                         *
     * nwpw_adaptive_scf_mixing::reset_for_restart
     *                                         *
     *******************************************/
    /**
     * @brief Reset adaptive mixing state for restart calculations.
     * This ensures that restart calculations start with clean mixing parameters.
     */
    void reset_for_restart() {
        reset_parameters();
        if (verbose_output && parall->is_master()) {
            std::cout << "        - Adaptive mixing reset for restart calculation" << std::endl;
        }
    }

    /*******************************************
     *                                         *
     * nwpw_adaptive_scf_mixing::get_nan_count
     *                                         *
     *******************************************/
    /**
    ...
    /*******************************************
     *                                         *
     * nwpw_adaptive_scf_mixing::set_verbose
     *                                         *
     *******************************************/
    /**
