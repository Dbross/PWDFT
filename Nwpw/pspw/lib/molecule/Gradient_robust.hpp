#ifndef GRADIENT_ROBUST_HPP
#define GRADIENT_ROBUST_HPP
...
/**
 * @brief Robust gradient computation with enhanced numerical stability
 * 
...
public:
    Gradient_robust(Pneb *grid, Molecule *molecule) 
        : mygrid(grid), mymolecule(molecule) {}
    ...
    double compute_gradient_robust(double *G1) {
        ...
        double total_energy = mymolecule->psi_1get_Tgradient(G1);
    ...
    double compute_gradient0_robust(double *G1) {
        ...
        double total_energy = mymolecule->psi_1get_Tgradient0(G1);
    ...
    /**
     * @brief Robust gradient computation with Stiefel manifold constraints
     */
    double compute_TSgradient_robust(double *G1) {
        // Compute Stiefel gradient
        double total_energy = mymolecule->psi_1get_TSgradient(G1);
