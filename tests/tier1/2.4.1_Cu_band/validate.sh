...
if ! grep -q ">>> job completed at" cu_band.out; then
  echo "FAIL: Calculation did not complete successfully."
  exit 1
fi

# 2. Check for band calculation output
if ! grep -q "task band energy" cu_band.out; then
  echo "FAIL: Band calculation task not found in output."
  exit 1
fi

# 3. Extract final total energy
FINAL_ENERGY=$(grep "total     energy" cu_band.out | tail -1 | awk '{print $4}')
